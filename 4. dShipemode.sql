...
-- 3. The combination of the quantity of ship mode, profit and sales between 2014,2015,2016 and 2017 by customer by using CTE and UNION

WITH ShipModeCustomerProfitSales2014 AS (
	SELECT  DISTINCT
		[YearOfOrder] as [Year],
 ...
 ) , ShipModeCustomerProfitSales2015 AS (
	SELECT  DISTINCT
		[YearOfOrder] as [Year],
		s.[Ship Mode], COUNT(c.[Customer Name]) OVER (Partition by s.[Ship Mode]) as [Quantity of Customer],
		SUM(fd.[SalesAmount]) OVER (PARTITION BY s.[Ship Mode]) as Sales,
 ...
 ) , ShipModeCustomerProfitSales2016 AS (
	SELECT  DISTINCT
		[YearOfOrder] as [Year],
		s.[Ship Mode], COUNT(c.[Customer Name]) OVER (Partition by s.[Ship Mode]) as [Quantity of Customer],
		SUM(fd.[SalesAmount]) OVER (PARTITION BY s.[Ship Mode]) as Sales,
 ...
 ) , ShipModeCustomerProfitSales2017 AS (
SELECT  DISTINCT
	[YearOfOrder] as [Year],
	s.[Ship Mode], COUNT(c.[Customer Name]) OVER (Partition by s.[Ship Mode]) as [Quantity of Customer],
	SUM(fd.[SalesAmount]) OVER (PARTITION BY s.[Ship Mode]) as Sales,
...
SELECT *
FROM ShipModeCustomerProfitSales2014
UNION
SELECT *
FROM ShipModeCustomerProfitSales2015
UNION
SELECT *
FROM ShipModeCustomerProfitSales2016
UNION
SELECT *
FROM ShipModeCustomerProfitSales2017
